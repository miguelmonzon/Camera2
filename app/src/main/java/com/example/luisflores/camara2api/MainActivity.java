...
//    To set camera for still pictures
    ...
    private class ImageSaver implements Runnable {
        ...
        public ImageSaver(final Image image) {
            mImage = image;
        }
            ...
            switch (mCaptureState) {
                ...
                case STATE_WAIT_LOCK:
                    ...
                    if (afState == CaptureResult.CONTROL_AF_STATE_FOCUSED_LOCKED || afState == CaptureResult.CONTROL_AF_STATE_NOT_FOCUSED_LOCKED) {
                        Toast.makeText(MainActivity.this, "AF Locked from Preview!!", Toast.LENGTH_SHORT).show();
        ...
        private void process(CaptureResult captureResult) {
            switch (mCaptureState) {
                ...
                case STATE_WAIT_LOCK:
                    ...
                    Integer afState = captureResult.get(CaptureResult.CONTROL_AF_STATE);
                    if (afState == CaptureResult.CONTROL_AF_STATE_FOCUSED_LOCKED || afState == CaptureResult.CONTROL_AF_STATE_NOT_FOCUSED_LOCKED) {
                        Toast.makeText(MainActivity.this, "AF Locked from Recording!!", Toast.LENGTH_SHORT).show();
                        startStillCaptureRequest();
                    }
                    break;
            }
...
//    Location for the still pictures to be saved
...
//                Finding out if we are in Portrait Mode. If so, we switch width for height and the other way around
    ...
    private void startStillCaptureRequest() {
        try {
            ...
            if (mIsRecording) {
                //We pass null for the background thread because we are already in the background thread
...
//    Stop background thread
    ...
    private void createImageFolder() {
        ...
        mImageFolder = new File(imageFile, "PicturesCameraAPI2");
